...
def local_variance(array, block_size=15):
    ...
    n = h.sum()
    n1 = n - 1
    d1 = cv2.filter2D(array ** 2, -1, h / n1, borderType=cv2.BORDER_REFLECT)
    d2 = cv2.filter2D(array, -1, h, borderType=cv2.BORDER_REFLECT) ** 2 / (n * n1)
    J = np.sqrt(np.maximum(d1 - d2, 0))

    return J


# 文件读取
image_path = 'D:/Desktop/test/dof'
file_names = os.listdir(image_path)
image_files = [f for f in file_names if any(ext in f.lower() for ext in ('.jpg', '.jpeg', '.png', '.bmp'))]
dof_img = cv2.imread(os.path.join(image_path, image_files[0]), 0)
count = 0
# 打开并显示每张图片
...
cv2.imwrite("D:/Desktop/test/dof/DOF.bmp", dof_img)
