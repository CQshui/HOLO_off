...
for image_name in image_files:
    ...
    try:
        ...
        for i in range(len(coeff1)):
            ...
            if (i == 0):
                fusedCooef.append(fuseCoeff(coeff1[0], coeff2[0], FUSION_METHOD2))
            else:
                # For the rest of the levels we have tupels with 3 coefficients
                c1 = fuseCoeff(coeff1[i][0], coeff2[i][0], FUSION_METHOD2)
                c2 = fuseCoeff(coeff1[i][1], coeff2[i][1], FUSION_METHOD2)
                c3 = fuseCoeff(coeff1[i][2], coeff2[i][2], FUSION_METHOD2)
