...
class AngularBatch(object):
    ...
    def reconstruct(self, img_pth, save_pth):
        ...
        fft_pth = os.path.join(save_pth, 'FFT_0_0.jpg')
        ...
        # 继续重建
        k = 2 * np.pi / self.lam
        fx = np.linspace(-1 / (2 * self.pix), 1 / (2 * self.pix), self.width)
        fy = np.linspace(-1 / (2 * self.pix), 1 / (2 * self.pix), self.height)
        FX, FY = np.meshgrid(fx, fy)
        temp = 1 - ((self.lam * FX) ** 2 + (self.lam * FY) ** 2)
        temp[temp < 0] = 0

        for i in range(len(self.z)):
            g = np.exp(1j * (2 * np.pi / self.lam) * self.z[i] * np.sqrt(temp))
            g[temp < 0] = 0
            g = fftshift(g)
...
if __name__ == '__main__':
    image = AngularBatch(lam=532e-9,
                         ...
                         z2=0.00020,
                         z_interval=0.00001,
                         input_pth=r'C:\Users\d1009\Desktop\test\origin\start',
                         output_pth=r'C:\Users\d1009\Desktop\test\temp')
    image.start()